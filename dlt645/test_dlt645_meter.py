...
def decode_dlt645(data):
    ...
    if ord(data[9]) > 0:
        d_out = ''
        for i in range(10, 10+ord(data[9])):
...
def dlt645_get_addr(serial):
    ...
    # try:
    cmd2 = b'\xfe\xfe\xfe\xfe\x68\xaa\xaa\xaa\xaa\xaa\xaa\x68\x13\x00\xdf\x16'

    # cmd2 = b'\xfe\xfe\xfe\xfe'
    # cmd_encode = encode_dlt645('\xaa\xaa\xaa\xaa\xaa\xaa', 0x13, 0, '')
...
def read_dlt645_once(serial_port, baud_rate):
    try:
        ...
        s = serial.Serial(serialport_path, serialport_baud,
                          parity=serial.PARITY_EVEN, timeout=0.1)
        # print s
...
if __name__ == '__main__':
    ret, f1, f2, f3 = read_dlt645_once('COM1', 1200)
    # print(ret, f1, f2, f3)

    # try:
    #     print("TRY SERIAL")
    #     serialport_baud = int(sys.argv[1])
    #     print(serialport_baud)
    #     serialport_path = sys.argv[2]
    #     print(serialport_path)
    #     s = serial.Serial(serialport_path, serialport_baud,
    #                       parity=serial.PARITY_EVEN, timeout=0.1)
