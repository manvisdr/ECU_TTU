...
broker = '127.0.0.1'
port = 1883
topic = "DaqoJc/notify/event/database/ADC/ADC_6536107f849fb0a4"
client_id = f'TTU-104'
...
class IEC60870_5_104_server:
    ...
    def GI_h(self, param, connection, asdu, qoi):
        ...
        if (qoi == 20):  # { /* only handle station interrogation */
            ...
            io = cast(MeasuredValueScaled_create(None, 100, -1,
                      IEC60870_QUALITY_GOOD), InformationObject)

            CS101_ASDU_addInformationObject(newAsdu, io)

            CS101_ASDU_addInformationObject(newAsdu,
                                            cast(MeasuredValueScaled_create(cast(
                                                io, MeasuredValueScaled), 101, 23, IEC60870_QUALITY_GOOD), InformationObject)
                                            )

            CS101_ASDU_addInformationObject(newAsdu,
                                            cast(MeasuredValueScaled_create(cast(
                                                io, MeasuredValueScaled), 102, 2300, IEC60870_QUALITY_GOOD), InformationObject)
                                            )
            ...
            io = cast(SinglePointInformation_create(
                None, 104, True, IEC60870_QUALITY_GOOD), InformationObject)

            CS101_ASDU_addInformationObject(newAsdu, io)

            CS101_ASDU_addInformationObject(newAsdu,
                                            cast(SinglePointInformation_create(cast(
                                                io, SinglePointInformation), 105, False, IEC60870_QUALITY_GOOD), InformationObject)
                                            )
            ...
            io = cast(SinglePointInformation_create(
                None, 300, True, IEC60870_QUALITY_GOOD), InformationObject)

            CS101_ASDU_addInformationObject(newAsdu, io)
            CS101_ASDU_addInformationObject(newAsdu, cast(SinglePointInformation_create(cast(
                io, SinglePointInformation), 301, False, IEC60870_QUALITY_GOOD), InformationObject))
            CS101_ASDU_addInformationObject(newAsdu, cast(SinglePointInformation_create(cast(
                io, SinglePointInformation), 302, True, IEC60870_QUALITY_GOOD), InformationObject))
            CS101_ASDU_addInformationObject(newAsdu, cast(SinglePointInformation_create(cast(
                io, SinglePointInformation), 303, False, IEC60870_QUALITY_GOOD), InformationObject))
            CS101_ASDU_addInformationObject(newAsdu, cast(SinglePointInformation_create(cast(
                io, SinglePointInformation), 304, True, IEC60870_QUALITY_GOOD), InformationObject))
            CS101_ASDU_addInformationObject(newAsdu, cast(SinglePointInformation_create(cast(
                io, SinglePointInformation), 305, False, IEC60870_QUALITY_GOOD), InformationObject))
            CS101_ASDU_addInformationObject(newAsdu, cast(SinglePointInformation_create(cast(
                io, SinglePointInformation), 306, True, IEC60870_QUALITY_GOOD), InformationObject))
            CS101_ASDU_addInformationObject(newAsdu, cast(SinglePointInformation_create(cast(
                io, SinglePointInformation), 307, False, IEC60870_QUALITY_GOOD), InformationObject))
        ...
        if (CS101_ASDU_getTypeID(asdu) == C_SC_NA_1):
            ...
            if (CS101_ASDU_getCOT(asdu) == CS101_COT_ACTIVATION):
                ...
                if (InformationObject_getObjectAddress(io) == 5000):
                    ...
                    print(
                        f"IOA: {InformationObject_getObjectAddress(io)} switch to {SingleCommand_getState(sc)}")
    ...
    def raw_msg(self, param, connection, p_msg, size, sent):
        print(f"{size}")
    ...
    def __init__(self, ip="0.0.0.0"):
        ...
        self.connectionRequestHandler = CS104_ConnectionRequestHandler(
            self.Conn_req)
        self.connectionEventHandler = CS104_ConnectionEventHandler(
            self.Conn_event)
        ...
        CS104_Slave_setServerMode(
            self.slave, CS104_MODE_SINGLE_REDUNDANCY_GROUP)
        ...
        CS104_Slave_setClockSyncHandler(
            self.slave, self.clockSyncHandler, None)

        # /* set the callback handler for the interrogation command */
        CS104_Slave_setInterrogationHandler(
            self.slave, self.interrogationHandler, None)
        ...
        CS104_Slave_setConnectionRequestHandler(
            self.slave, self.connectionRequestHandler, None)

        # /* set handler to track connection events (optional) */
        CS104_Slave_setConnectionEventHandler(
            self.slave, self.connectionEventHandler, None)

        # /* uncomment to log messages */
        CS104_Slave_setRawMessageHandler(
            self.slave, self.rawMessageHandler, None)
    ...
    def start(self):
        ...
        while True:
            time.sleep(1)
            newAsdu = CS101_ASDU_create(
                self.alParams, False, CS101_COT_PERIODIC, 0, 1, False, False)
            io = cast(MeasuredValueScaled_create(
                None, 110, scaledValue, IEC60870_QUALITY_GOOD), InformationObject)
