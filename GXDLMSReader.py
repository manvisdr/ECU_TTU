...
class GXDLMSReader:
    ...
    def readDLMSPacket2(self, data, reply):
        ...
        with self.media.getSynchronous():
            if not reply.isStreaming():
                self.writeTrace("TX: " + self.now() + "\t" +
                                GXByteBuffer.hex(data), TraceLevel.VERBOSE)
            ...
            try:
                ...
                self.writeTrace("RX: " + self.now() + "\t" +
                                str(rd), TraceLevel.ERROR)
                raise e
            self.writeTrace("RX: " + self.now() + "\t" +
                            str(rd), TraceLevel.VERBOSE)
            # if reply.error != 0:
    ...
    def initializeOpticalHead(self):
        if self.client.interfaceType == InterfaceType.HDLC_WITH_MODE_E:
            ...
            # Set mode E.
            tmp = bytearray([0x06, controlCharacter, ord(
                baudrate), modeControlCharacter, 13, 10])
            p.reply = None
            with self.media.getSynchronous():
                self.media.send(tmp)
                ...
                time.sleep(1)
                self.writeTrace("TX: " + self.now() + "\t" +
                                GXCommon.toHex(tmp), TraceLevel.VERBOSE)
                p.waitTime = 200
        ...
        if self.client.authentication > Authentication.LOW:
            try:
                ...
                raise GXDLMSException(
                    AssociationResult.PERMANENT_REJECTED, SourceDiagnostic.AUTHENTICATION_FAILURE)
    ...
    def getProfileGenericColumns(self):
        ...
        for pg in profileGenerics:
            self.writeTrace("Profile Generic " + str(pg.name) +
                            "Columns:", TraceLevel.INFO)
            try:
                if pg.canRead(3):
                    self.read(pg, 3)
                if self.trace > TraceLevel.WARNING:
                ...
                self.writeTrace("Err! Failed to read columns:" +
                                str(ex), TraceLevel.ERROR)
    ...
    def getReadOut(self):
        ...
        for it in self.client.objects:
            ...
            self.writeTrace("-------- Reading " + str(it.objectType) +
                            " " + str(it.name) + " " + it.description, TraceLevel.INFO)
            for pos in it.getAttributeIndexToRead(True):
                try:
                    if it.canRead(pos):
                        ...
                        self.writeTrace("Attribute" + str(pos) +
                                        " is not readable.", TraceLevel.INFO)
                except Exception as ex:
                    self.writeTrace("Error! Index: " + str(pos) +
                                    " " + str(ex), TraceLevel.ERROR)
        ...
        if isinstance(val, (bytes, bytearray)):
            ...
            for tmp in val:
                ...
                if isinstance(tmp, bytes):
                    ...
                    str_ += str(tmp)
            val = str_
        self.writeTrace("Index: " + str(pos) + " Value: " +
                        str(val), TraceLevel.INFO)
    ...
    def getProfileGenerics(self):
        ...
        for it in profileGenerics:
            ...
            self.writeTrace("Entries: " + str(entriesInUse) +
                            "/" + str(entries), TraceLevel.INFO)
            pg = it
                    ...
                    for rows in cells:
                        for cell in rows:
                            if isinstance(cell, bytearray):
                                self.writeTrace(GXByteBuffer.hex(
                                    cell) + " | ", TraceLevel.INFO)
                            else:
                                self.writeTrace(
        ...
        if not self.client.useLogicalNameReferencing:
            ...
            if sn and sn.version > 0:
                try:
                    ...
                    self.writeTrace(
                        "Access rights are not implemented for the meter.", TraceLevel.INFO)
    ...
    # def ClientDisconnectControl(self, outputFile):
    #     try:
    #         read = False
    #         self.initializeConnection()
    #         if outputFile and os.path.exists(outputFile):
