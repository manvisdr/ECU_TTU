        ...
        if 'DLMS' in msg.topic:
            ...
            if tempDLMS['body'][0]['val'] not in dlmsId:
                ...
                for v in tempDLMS['body']:
                    if v['name'] == 'pos_kwh':
                        dlmsKWH.append(
                            [tempDLMS['body'][0]['val'], int(v['val'])/10])
                        dlmsKWHLast.append(
                            [tempDLMS['body'][0]['val'], int(v['val'])/10])
            else:
                for v in tempDLMS['body']:
        ...
        if 'LVMeter' in msg.topic:
            ...
            if tempLV['body'][0]['val'] not in LVId:
                ...
                for v in tempLV['body']:
                    if v['name'] == 'kwhtotal':
                        LVKWH.append(
                            [tempLV['body'][0]['val'], int(v['val'])/100])
                        LVKWHLast.append(
                            [tempLV['body'][0]['val'], int(v['val'])/100])
                        lineloss_array.append(0)
                        print(LVKWH)
            else:
                for v in tempLV['body']:
            ...
            for i in range(len(LVKWH)):
                ...
                if a == 0:
                    ...
                    loss_val = {
                        "name": "lossVal",
                        "val": abs(LVEnergy-dlmsKWH_adding)/1000,
                        "timestamp": dateTimestamp()
                    }
                    lvEnergyAll = {
                        "name": "branchEnergy",
                    ...
                    linelossrate = {
                        "name": "lineloss",
                        "val": abs(lineLossFormula/10),
                        "timestamp": dateTimestamp()
                    }
                    bodyValue = [loss_val, lvEnergyAll,
                                 meterEnergy, linelossrate]
                    lineloss_array[i] = linelossrate
                ...
                # if len(LVKWH) == 2 and lineloss_array[i] < lineloss_array[i-1]:
