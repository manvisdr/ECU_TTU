...
def clientSettings(dicts=dict):
    ...
    if client.serverAddress != 1:
        ...
        client.serverAddress = int(v['serverAddress'])
    client.serverAddress = GXDLMSClient.getServerAddressFromSerialNumber(
        20032741)

    # client.serverAddress = 0x4000 | aa % (10000 + 1000)
    client.serverAddress = 3741
    print(client.serverAddress)
    if v['interface'].lower() == "HDLC".lower():
        print("INTERFACE : HDLC")
        client.interfaceType = InterfaceType.HDLC
    elif v['interface'].lower() == "WRAPPER".lower():
        print("INTERFACE : WRAPPER")
        client.interfaceType = InterfaceType.WRAPPER
    elif v['interface'].lower() == "HdlcWithModeE".lower():
        print("INTERFACE : HDLC_WITH_MODE_E")
        client.interfaceType = InterfaceType.HDLC_WITH_MODE_E
    elif v['interface'].lower() == "Plc".lower():
        print("INTERFACE : PLC")
        client.interfaceType = InterfaceType.PLC
    elif v['interface'].lower() == "PlcHdlc".lower():
